...
import org.ardenus.engine.input.device.event.DeviceHoldEvent;
import org.ardenus.engine.input.device.event.DevicePressEvent;
import org.ardenus.engine.input.device.event.DeviceReleaseEvent;
...
public class Keyboard extends InputDevice {
	...
	private void pollKey(long time, KeyboardKey key) {
		...
		if (!wasPressed && pressed) {
			Input.sendEvent(new DevicePressEvent(this, key, null, false));
			state.pressTime = time;
		} else if (wasPressed && !pressed) {
			Input.sendEvent(
					new DeviceReleaseEvent(this, key, null, state.held));
			state.held = false;
		}
		...
		if (state.pressed) {
			if (!state.held && time - state.pressTime >= 1000L) {
				Input.sendEvent(new DeviceHoldEvent(this, key, null));
				state.held = true;
			}

			if (state.held && time - state.lastHeldPress >= 100L) {
				Input.sendEvent(new DevicePressEvent(this, key, null, true));
