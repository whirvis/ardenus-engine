...
public abstract class Controller extends InputDevice {
	...
	private void pollState(long currentTime, PressableState state,
		...
		if (!wasPressed && pressed) {
			Input.sendEvent(
					new DevicePressEvent(this, feature, direction, false));
			state.pressTime = currentTime;
		} else if (wasPressed && !pressed) {
			Input.sendEvent(new DeviceReleaseEvent(this, feature, direction,
					state.held));
			state.held = false;
		}
		...
		if (state.pressed) {
			...
			if (state.held && currentTime - state.lastHeldPress >= 100L) {
				Input.sendEvent(new DevicePressEvent(this, feature,
						direction, true));
